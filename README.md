...
## Фаза 11 — профили, эмодзи, мелочи

- **Профили с вкладками:** Записи / Фото / Друзья / Сообщества; «Фото» — сетка картинок из постов с галереей в лайтбоксе.
- **Живой профиль:** онлайн-статус («в сети / был(а)…»), кольцо истории на аватаре (клик открывает истории), общие друзья, бейджи (🌱 новичок / ✍️ автор / 👑 владелец сообщества), закреплённый пост (📌).
- **Эмодзи-пикер** в чате и композере постов.
- **Скелетоны загрузки** на лентах/чатах/профиле (плавные переходы).
- **Уведомления** о лайках/комментариях ведут на конкретный пост (`/posts/[id]`).
- **Аватар групповой беседы** — загрузка при создании и в настройках беседы.
- Исправлен баг просмотрщика историй (кадр пропадал при открытии из-за рефреша).

## Фаза 12 — реалтайм, бренд, анимации

- **Реалтайм-сообщения:** список чатов, лента и счётчики в навигации обновляются сами (умный поллинг `LiveRefresh` — `router.refresh` пока вкладка видима); новые сообщения видны без перезагрузки. Окно чата по-прежнему опрашивает сервер каждые 2 сек.
- **Бренд mesto:** фирменный знак-пин («место» + чат-пузырь) с буквой «m» — в шапке и на страницах входа/регистрации; анимированный логотип.
- **Анимации (framer-motion):** плавные переходы между страницами, появление сообщений в чате (spring), скользящий индикатор вкладок (профиль/поиск), пульс онлайн-статуса, пружинные бейджи навигации, микро-интеракции. Всё с уважением к `prefers-reduced-motion`.
